<template>
  <div class="top_box clearfix">
    ...
    <div class="person" v-if="user!='' && types!='doc' && types!='detail'">
      <span @click.stop="personSliderFn"><span class="img_box photo"  v-if="is_default_headimg == 1">{{user.title.substring(0,1)}}</span><span class="img_box photo" :class="{white: is_default_headimg != 1}" v-if="is_default_headimg != 1"><img class="img" :src="headerImg" alt=""></span><i class="icon icon-bottom"></i></span>
    ...
    <div class="logo_box" v-if="types=='report'"><router-link to="/"><img src="../assets/img/logo.png" class="logo" />细软图思</router-link><span class="txt">商标监测报告</span></div>
    <div class="logo_box" v-if="types=='jgg'"><router-link to="/"><img src="../assets/img/logo.png" class="logo" />细软图思</router-link><span class="txt">商标九宫格</span></div>
    <div class="logo_box" v-if="types=='doc'"><img src="../assets/img/logo.png" class="logo" />细软图思<span class="txt">商标法律文书详情</span></div>
    <div class="logo_box" v-if="types=='detail'"><router-link to="/"><img src="../assets/img/logo.png" class="logo" />细软图思</router-link><span class="txt">商标详情</span></div>
    <div class="logo_box" v-if="types=='team'"><img src="../assets/img/logo.png" class="logo" />细软图思<span class="txt">团队</span></div>
    <div class="nav_box" v-if="types=='work' || types=='brand' || types=='monitor' || types == 'qzh' || types == 'calendar'">
      <div class="left">
        <img src="../assets/img/logo.png" class="logo" /><span class="txt" @click.stop="sliderFn">{{curObj.name}}<i class="icon icon-bottom"></i></span>
        <div class="slider_box" v-if="teamShow" @click.stop="teamShowFn">
          ...
          <div class="section">
            <router-link class="list" to="/team/new">加入新团队</router-link>
            <router-link to="/team/list" class="list">管理团队</router-link>
          </div>
        </div>
      </div>
      <!-- <ul class="nav clearfix">
        <li :class="{cur:navIndex==0}"><router-link :to="'/work/index?id='+orgid">工作台</router-link></li>
        <li :class="{cur:navIndex==1}"><router-link to="/group/frame">团队大厅</router-link></li>
        <li :class="{cur:navIndex==2}"><router-link to="/report/brand">商标分析</router-link></li>
        <li :class="{cur:navIndex==3}"><router-link to="/search/index">商标查询</router-link></li>
        <li :class="{cur:navIndex==4}"><router-link to="/monitor/index">商标监测</router-link></li>
        <li :class="{cur:navIndex==6}"><router-link to="/calendar/index">商标雷达</router-link></li>
        <li :class="{cur:navIndex==5}"><router-link to="/qzh/index">千纸鹤</router-link></li>
      </ul> -->
    </div>
    ...
    <!--/意见反馈-->
    <!--通知消息-->
    <div class="message_layer layer" v-if="messageShow">
      <a href="javascript:void(0)" class="close" @click="closeFn"><i class="icon icon-close"></i></a>
      <div class="tit">通知消息<span class="btn" @click="updateMsgFn"><i class="icon icon-zidongqingli"></i>全部已读</span><span class="btn" @click="delShowFn"><i class="icon icon-del"></i>清空已读</span></div>
      <div class="main_box clearfix">
        <div class="part_1">
          <ul v-if="msgList.length">
            <li :class="[{checked:!i.is_read,cur:i.cur}]" v-for="(i,index) of msgList" :key="index" @click="tabFn(index)"><!--is_read=0未读--is_read=1已读-->
              <div class="msg_tit"><span class="right"><span class="time">{{i.leave}}</span><span class="icon icon-del" @click="delFn(i.msg_id)"></span></span><span class="txt">监测通知</span></div>
              <div class="msg">有{{i.nums}}个任务监测到商标更新</div>
            </li>
          </ul>
          <div class="empty_box" v-else><i class="icon icon-xiaoxitongzhitixinglingshenglingdang"></i><div class="txt">还未收到任何消息</div></div>
        </div>
        <div class="part_2">
          <div class="detail_list_box">
            <div class="loading_box" v-if="logLoading">
    ...
    <!--/通知消息-->
    <!--清空已读消息-->
      <div class="layer w420 del_msg_layer" v-if="delShow">
          <a href="javascript:void(0)" class="close" @click="closeFn('del')"><i class="icon icon-close"></i></a>
          <div class="tit">清空已读消息</div>
          <div class="txt">确定要清空所有已读消息吗？</div>
          <div class="btn_box">
              <a href="javascript:void(0)" class="btn_1" @click="closeFn('del')">取消</a>
              <span class="err">{{err}}</span>
              <a href="javascript:void(0)" class="btn_2" @click="delAllFn">确定</a>
          </div>
      </div>
      <!--/清空已读消息-->
      <!--个人资料-->
          ...
          <ul class="ul">
            <li class="li" v-for="(item, index) in downList" :key="index"  >
              ...
              <div class="edit-box" >
                ...
                <span btn @click.stop="downDelLayerShowFn(index)"  v-show="item.status == 3 || item.status == 4"  class="btn">删除</span>
...
.layer{
  ...
  &.message_layer{
    width:980px;height: 500px;padding:14px;
    .empty_box{
      color:#A2A7B0;
      .icon{font-size: 40px;}
    }
    .tit{
      .btn{
        color: #A2A7B0;font-size: 12px;font-weight: normal;margin-left:30px;cursor: pointer;
        .icon{
          vertical-align: top;font-size: 20px;margin-right: 2px;
        }
        &:hover{
          color:#333;
          .icon{
    ...
    .part_1{
      background: #ECEEF2;border-radius: 4px;height:432px;width:290px;overflow-y: scroll;
      box-sizing: border-box;padding:10px 4px 10px 10px;
      li{
        background: #fff;border-radius: 4px;border-left: 4px solid #fff;overflow: hidden;margin-bottom: 10px;cursor:pointer;
        &:hover{
        ...
        .msg{
          font-size: 12px;color: #A2A7B0;line-height: 22px;padding:0 0 10px 8px;
        }
        .msg_tit{
          line-height: 40px;padding:0 10px 0 8px;color:#A2A7B0;
          .right{
            float: right;
            .time{
              font-size: 12px;
            ...
            .icon{
              vertical-align: top;font-size:20px;display: none;
            }
          }
        }
      }
    ...
    .part_2{
      width:648px;margin-left: 14px;border-top: 1px solid #e9e9e9;height:410px;padding-bottom:20px;overflow-y:scroll;
      .detail_list_box{
        li{
          .name{
            padding-top:14px;line-height: 20px;color: #A2A7B0;
          }
          .detail{
            display: block;line-height: 20px;padding:10px; border:1px solid #e9e9e9;border-radius:4px;margin-top:10px;
            ...
            .btn{
              ...
              .icon{
                color: #A2A7B0;font-size: 20px;vertical-align: top;
              }
              &.gray{
                cursor: default;color: #A2A7B0;
                &:hover{
                  color: #A2A7B0;
                }
              }
            ...
            .tag{
              display: inline-block;width: 50px;line-height: 20px;border-radius: 4px;
              background: #ECEEF2;color: #A2A7B0;text-align: center;font-size: 12px;
            }
            .txt{padding-left: 14px;}
            .msg{
              color: #FB717E;padding-left: 25px;
            }
          }
        }
      }
...
.top_box{
  ...
  .area_box{
    ...
    textarea{
      background: #ECEEF2;width:100%;height: 100px;box-sizing: border-box;padding:6px;line-height: 22px;
    }
  }
  .slider_box{
      position: absolute;background: #fff;box-shadow:0px 0px 10px #E1E6EF;width:200px;cursor:default;z-index: 6;
      .section{
        border-bottom: 1px solid #e9e9e9;padding:4px 0;width:100%;box-sizing:border-box;
        &.scroll{
        ...
        .name,.option,.list{
          ...
          &.list{
            cursor: pointer;display: block;line-height:34px;
            &:hover{
              background: #ECEEF2;
            }
          }
  ...
  .message_box{
    float: right;line-height:54px;position: relative;
    .icon{
      font-size: 22px;color: #A2A7B0;vertical-align: top;margin-right: 25px;
    }
    .circle{
      position: absolute;background: #FB717E;width:8px;height: 8px;border-radius: 50%;top:12px;left:22px;
      &.green{
  ...
  .person{
    float: right;color: #7F7F7F;cursor:pointer;position: relative;z-index: 5;height: 55px;
    .slider_box{
      right:0;top:48px;
    }
    .img_box{
    ...
    .icon{line-height: 55px;vertical-align: top;font-size: 12px;color: #aaa;}
  }
  .nav_box,.logo_box{
    .logo{width:34px;height: 34px;margin-top:8px;vertical-align: top}
  }
  .nav_box{
    .left{
      float: left;padding-right:30px;position: relative;z-index: 5;
      .txt{
        font-size: 18px;font-weight: bold;line-height: 30px;padding:0 5px;cursor: pointer;margin:10px 0 0 10px;display: inline-block;
        border-radius: 4px;
        &:hover{
          background: #ECEEF2;
        }
      }
      .icon{color: #aaa;font-size: 14px;margin-left:8px;}
      .slider_box{
        left:0;top:56px;width:260px;
        .list{
          padding:0 15px;
    ...
    .nav{
      ...
      li{
        ...
        &.cur{
          a{
            border-bottom: 2px solid #D9001B;color: #D9001B;
          }
        }
      }
    }
  ...
  .logo_box{
    ...
    .txt{
      display: inline-block;line-height: 34px;border-left:1px solid #e9e9e9;font-size: 20px;font-weight: bold;
...
.down-layer{
  ...
  right: 100px; top:40px; width: 320px; padding:15px 0;
