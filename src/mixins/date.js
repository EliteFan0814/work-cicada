module.exports = {
  methods: {
    ...
    getWeekFn(date) {
      ...
      for (let i = 1; i <= 7; i++) {
        ...
        week.push(
          tim.getFullYear() +
            '-' +
            String(tim.getMonth() + 1).padStart(2, '0') +
            '-' +
            String(tim.getDate()).padStart(2, '0')
        )
      }
    ...
    getTimeFn(month = '', year = '') {
      ...
      if (date.year % 4 == 0) {
        for (let i = 1; i <= month_olympic[date.month - 1]; i++) {
          ...
          const day = `${date.year}-${date.month}-${i}`
          monthList.push(day)
        }
      } else {
      ...
      if (date.month >= 2) {
        if (year % 4 == 0) {
          prevMonthList = this.getPrevMonthFn(
            month_olympic[date.month - 2],
            startDate,
            date.month - 1,
            date.year
          )
      ...
      const monthShow = { month: date.month, year: date.year }
      const prevMonth = {
      ...
      if (date.month == 12) {
        nextMonthList = this.getNextMonthFn(
          prevMonthList.length,
          monthList.length,
          1,
          date.year + 1
        )
    ...
    getMonthStartFn(month, year) {
      const tmDate = new Date(year, month - 1, 1)
